...
from wenet.transformer.LexiconTrie import lexicon_beam_search, beam_search
...
class ASRModel(torch.nn.Module):
    ...
    def _ctc_prefix_beam_search(
        ...
        hyps = beam_search(ctc_probs, beam_size, )
    ...
    def attention_rescoring(
        ...
        hyps_pad = pad_sequence([torch.tensor(hyp[0], device=device, dtype=torch.long) for hyp in hyps], True, self.ignore_id)  # (beam_size, max_hyps_len)
    ...
    def lexicon_attention_rescoring(
        self,
        speech: torch.Tensor,
        speech_lengths: torch.Tensor,
        beam_size: int,
        ...
        hyps, encoder_out = self._ctc_prefix_beam_search(
            speech, speech_lengths, beam_size, decoding_chunk_size,
            num_decoding_left_chunks, simulate_streaming, lexicon)
        if len(hyps) == 0:
            return (5, )
        # pad_hyps = [((self.ignore_id,), -100)] * (beam_size - len(hyps))
        # hyps.extend(pad_hyps)
        beam_size = len(hyps)
    ...
    def forward_attention_decoder(
        ...
        decoder_out, _ = self.decoder(encoder_out, encoder_mask, hyps, hyps_lens)  # (num_hyps, max_hyps_len, vocab_size)
        decoder_out = torch.nn.functional.log_softmax(decoder_out, dim=-1)
        return decoder_out
